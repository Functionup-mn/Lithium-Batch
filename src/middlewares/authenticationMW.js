...
const authenticate = function (req, res, next) {
  try {
    ...
    if (!token) {
      res.status(400).send({status: false, msg: "Please set x-api-key header" })
    }
...
const authorization = async function (req, res, next) {
  try {
    const blogId = (req.params.blogId || req.query.authorId.category.tags.subcategory)
    if (!blogId) {
      return res.status(400).send({ status: false, msg: "blogId is mandatory for authorization" })
    }
    ...
    if (decodedToken.authorId != getBlog.authorId.toString()) {
